...
- `artifacts/ai-chat` — Expo mobile app (AI Chat). User enters their own OpenRouter API key on first launch (stored in AsyncStorage), then chats. Multiple free models selectable. No backend, no database — direct calls to https://openrouter.ai/api/v1/chat/completions from the device.
  - Tools (function-calling, defined in `lib/tools.ts`, executed by `lib/openrouter.ts` tool loop, `MAX_TOOL_HOPS = 50`):
    ...
    - **Custom HTTP tools** (user-defined at runtime, `lib/customToolsStore.ts` + `lib/httpToolRunner.ts`, JSON files under `${documentDirectory}custom-tools/<name>.json`): users add their own REST tools via the chat header `zap` icon → `/tools` (list) → `/tools/[name]` (editor). Each tool has: `name` (function name, regex `^[a-z][a-z0-9_]{0,47}$`), `label` (status text), `description` (read by AI), `parameters[]` (`{name, type: string|number|integer|boolean, description, required, enumValues?}`), `request` (`method`, `url`, `headers`, `query`, `bodyType: none|json|form|text`, `bodyTemplate`, `timeoutMs`), `response` (`type: json|text`, `pick?: string[]` dot-notation, `maxChars?`). Placeholder syntax `{paramName}` substitutes args into url/headers/query values/bodyTemplate (URL-encoded for url/query). `lib/tools.ts` exposes `getAllTools()` / `getToolDefinitions()` / `findTool()` (all async — built-ins from `BUILTIN_TOOLS` + custom mapped via `buildToolDefinition`); custom tools whose name collides with a built-in are filtered out (`BUILTIN_NAMES` set). `getCustomToolsBrief()` returns `[{name, description}]` used by `app/chat.tsx` to dynamically append "Tools custom user" section to the system prompt each request, so the AI auto-discovers new tools without restart. `ToolEvent` includes `label` so the chat status text uses the tool's own label. Editor has a Test button that runs the tool with sample args and shows the actual response. Long-press list item to delete.
  ...
  - Tool loading status text comes from each tool's `label` field, surfaced in `ToolEvent.label` and shown by `app/chat.tsx` via `setToolStatus(event.label ?? ...)`.
...
## Mobile Build (APK only)

`artifacts/ai-chat/eas.json` is configured to produce **APK files only** (no AAB, no iOS) on Android via EAS Build. Three profiles:

- `development` — debug build with `developmentClient: true`, internal distribution, channel `development` (`APP_ENV=development`).
- `preview` — internal distribution APK, channel `preview` (`APP_ENV=preview`).
- `production` — APK with `autoIncrement` for versionCode, internal distribution, channel `production` (`APP_ENV=production`).

Run from `artifacts/ai-chat/`:
- `pnpm run eas:login` — log in to Expo first
- `pnpm run apk:development` / `pnpm run apk:preview` / `pnpm run apk:production` — build the corresponding APK on EAS

`app.json` keeps `android.package` = `com.aichat.app`. EAS `projectId` is **not** hardcoded — run `pnpm exec eas init` (after `pnpm run eas:login`) to create/link a project under the current Expo account; EAS will inject the `projectId` and `updates.url` automatically.
